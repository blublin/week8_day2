...
function orderedMultisetUnion(sortedA, sortedB) {
    ...
    let topInd = 0;
    let botInd = 0;
    while (topInd < sortedA.length && botInd < sortedB.length) {
        if (sortedA[topInd]===sortedB[botInd]){
            newArr.push(sortedA[topInd]);
            topInd++;
            botInd++;
        }
        else if (sortedA[topInd] < sortedB[botInd]){
            newArr.push(sortedA[topInd]);
            topInd++;
        }
        else {
            newArr.push(sortedB[botInd]);
            botInd++;
        }
    }
    // Fill  with leftover unused array
    while (topInd < sortedA.length) {
        newArr.push(sortedA[topInd]);
        topInd++;
    }
    while (botInd < sortedB.length) {
        newArr.push(sortedB[botInd]);
        botInd++;
    }
    return newArr;
